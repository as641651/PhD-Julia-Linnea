...
getrf_kernel = pkg_resources.resource_string(__name__,os.path.join(template_path,"getrf_kernel.jl")).decode("UTF-8")
...
c = 0
kernel_function = getrf_kernel.format()
measurements = ""
for exp in exps:
    c = c+1
    ...
    measurements += "\n"
    measurements += "\tmeasure_getrf({size},{property})\n".format(**args)
    measurements += "\n"
    if c%50 == 0:
        measurements += "\n"
...
f = open("experiments/getrf_experiments.jl","w")
f.write(code)
f.close()
